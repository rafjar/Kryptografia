def sha512(data):
    ...
    for offset in range(0, len(data), 128):
        ...
        # Break chunk into sixteen 128-bit = 8-bytes words w[i], 0 ≤ i ≤ 15
        for i in range(16):
            w[i] = int.from_bytes(chunks[8*i : (i+1)*8], byteorder='big')
    ...
    ret = bytes()
    for h in hash:
        ret += h.to_bytes(8, 'big')

    return ret.hex()

print(sha512('Ala ma kota'))
print(sha512('Ala ma kota1'))
print(sha512('Kryptografia jest super!'))